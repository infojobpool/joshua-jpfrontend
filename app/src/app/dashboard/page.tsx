                          ...
                          {task.cancel_status && task.cancelled_by_role === "tasker" ? (
                            ...
                            <>
                              <div className="flex flex-wrap gap-2 w-full items-center">
                                <Link href={`/tasks/${task.id}`} className="shrink-0" onMouseEnter={() => prefetchTask(task.id)} onTouchStart={() => prefetchTask(task.id)}>
                                  <Button
                                    variant="outline"
                    ...
                    <div className={isMobile ? "p-4" : "p-6"}>
                      ...
                      <div className={`flex flex-wrap items-center gap-2 mt-4`}>
                        {isCancelled ? (
                          ...
                          <>
                        <Link href={`/tasks/${task.id}`} className="shrink-0" onMouseEnter={() => prefetchTask(task.id)} onTouchStart={() => prefetchTask(task.id)} onClick={() => { try { sessionStorage.setItem("nav_from_assigned","1"); } catch {} }}>
                ...
                {sortedRequestedTasks.map((bid) => (
                  <Card key={bid.bid_id} className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-slate-800/80 dark:via-slate-800/60 dark:to-indigo-900/30 border-l-4 border-l-blue-500 dark:border-l-indigo-400 shadow-lg shadow-slate-200/40 hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 rounded-2xl overflow-hidden">
                    ...
                    <div className={isMobile ? "p-4" : "p-6"}>
                      ...
                      {/* Action button */}
                      <div className="mt-4">
                        <Link href={`/tasks/${bid.task_id}`} className="w-full" onMouseEnter={() => prefetchTask(bid.task_id)} onTouchStart={() => prefetchTask(bid.task_id)}>
